...
import { referralHistory, clients, adminNotifications } from "@/db/schema";
...
export async function getReferralSettings() {
    ...
    return Object.entries(DEFAULT_REFERRAL_SETTINGS).map(([k, v]) => ({ key: k, value: v }));
}

export async function updateReferralSettings(updates: { key: string; value: string }[]) {
    await requireAdminSession();
    // Implementation removed due to schema V2 changes deleting the 'settings' table
