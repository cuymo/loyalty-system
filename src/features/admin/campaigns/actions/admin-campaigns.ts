...
export async function getCrmClients() {
    ...
    const result = await db
        .select({
            ...
            wantsInAppNotifs: sql<boolean>`true`.as("wantsInAppNotifs"), // Simulated since V2 removed it
            ...
            codesRedeemed: sql<number>`(SELECT COUNT(*) FROM ${codes} WHERE ${codes.usedBy} = clients.id)`.mapWith(Number),
            lastVisit: sql<string | null>`(SELECT MAX(${codes.usedAt}) FROM ${codes} WHERE ${codes.usedBy} = clients.id)`,
...
export async function processCrmCampaign(
    ...
    return await db.transaction(async (tx) => {
        ...
        if (data.pointsToGift > 0) {
            ...
            const inAppNotifs = clientsList
                .map(c => ({
                    ...
                    type: "campaign_with_points" as const,
                    isRead: false
                }));
        ...
        if (data.title && data.body) {
            ...
            const inAppMessageNotifs = clientsList
                .map(c => ({
                    clientId: c.id,
